...
- **Milestone 5 (label generation + appeals layer):** I'll provide "#### 5. Transparency Label Generation", the label design variants from "### Summary Implementation Decisions", the full "#### 8. Appeals Workflow" section, and the API contract for `POST /appeals` and `PATCH /appeals/{content_id}`. With those, I'll ask it to generate:
    ...
    (2) the `POST /appeals` route that appends to `/logs/appeal_queue.jsonl` and logs an `appeal_submitted` audit event
    (3) the `PATCH /appeals/{content_id}` route that removes the resolved entry from the queue and logs an `appeal_resolved` audit event
I'll verify by:
    ...
    - Calling `POST /appeals` and checking that `/logs/appeal_queue.jsonl` gains a new line with the correct `content_id` and `status: pending_review`
    - Calling `PATCH /appeals/{content_id}` with a resolution and checking that the entry is gone from the queue and the audit log has an `appeal_resolved` event
